...
function setup() {
  ...
  canvas.parent('sketch'); // Make sure the canvas is placed in the 'sketch' div

  grundBild = createGraphics(width, height);
  grundBild.background("white");
  grundBild.noFill();
  grundBild.stroke(""); // Linienfarbe
...
// Add this function to update abstand from input field
function updateAbstand() {
  let inputValue = parseInt(document.getElementById('line-spacing').value);
  if (inputValue >= 5 && inputValue <= 50) {
    abstand = inputValue;
    mousePressed(); // Use existing mousePressed to reset simulation
  }
}

// Verbesserte updateRauschSkala Funktion
function updateRauschSkala() {
  let inputValue = parseFloat(document.getElementById('noise-scale').value);
  if (!isNaN(inputValue) && inputValue >= 0.01 && inputValue <= 0.2) {
    rauschSkala = inputValue;
    console.log("Rauschskala aktualisiert auf:", rauschSkala);
    
    // Wichtig: Setze einen neuen Noise-Seed
    noiseSeed(random(10000));
    
    mousePressed(); // Reset simulation
  } else {
    console.log("Ungültiger Wert für Rauschskala:", inputValue);
  }
}

function zeichneMusterLinie(y, rauschSkala, überstand) {
  // Log für Debugging
  console.log("Zeichne Linie mit rauschSkala:", rauschSkala);
...
function mousePressed() {
  console.log("Reset mit rauschSkala:", rauschSkala, "und abstand:", abstand);
  
  // Setze einen neuen Noise-Seed bei jedem Reset
  noiseSeed(random(10000));
  ...
  grundBild.background("white");
  grundBild.noFill();
  grundBild.stroke(0, 0, 0, 50); // Schwarze Linien mit etwas Transparenz
